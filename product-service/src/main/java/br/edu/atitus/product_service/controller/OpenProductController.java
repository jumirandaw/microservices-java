...
public class OpenProductController {
	...
	public ResponseEntity<ProductEntity> getProduct(@PathVariable Long idProduct, @PathVariable String targetCurrency)
		...
		String keyChache = idProduct + targetCurrency;

		ProductEntity product = cacheManager.getCache(nameCache).get(keyChache, ProductEntity.class);
		...
		if (product == null) {
			...
			if (product.getCurrency().equals(targetCurrency)) {
				...
				if (currency != null) {
					...
					cacheManager.getCache(nameCache).put(keyChache, product);
				} else {
					product.setConvertedPrice(-1);
					product.setEnviroment(product.setEnviroment() + " - Currency unavalaible");
				}
			}
